  ...
  privacy: {
    ...
    body: (
      <>
        ...
        <p><strong>Fault reports.</strong> When the interface hits an error, the browser sends the error message, the technical stack trace, the page you were on, and the browser type, so the fault can be fixed without asking you to reproduce anything. No page content, no form input, no identifiers, no tracking. Purpose: fixing faults. Retention: 14 days, then deleted.</p>
        <H>2. How your data is used, in one paragraph</H>
        <p>Your account data authenticates you. Session and security data keep your account safe and stop abuse. Voice audio converts and returns; the platform does not bank it. Clone sample audio sits in storage, private and queued, until real training capacity exists. Metering data keeps free limits and credit math honest. Attestation data protects the people whose voices could be cloned. Contact and waitlist data exist only to answer you or to tell you the app shipped. Fault reports exist only to fix what broke. Nothing on this list is used for advertising, profiling, or model training, and no data is sold.</p>
