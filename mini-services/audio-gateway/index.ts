...
//
// Outbound-only from the worker's perspective: the worker dials this service;
...
const httpServer = createServer((req, res) => {
  if (req.url === "/health") {
    ...
    res.end(JSON.stringify({ ok: true, sessions: pairs.size, time: new Date().toISOString() }));
    return;
  }
  res.writeHead(404);
  res.end();
...
httpServer.listen(PORT, () => {
  console.log(JSON.stringify({ level: "info", msg: "audio_gateway_listening", port: PORT }));
});
